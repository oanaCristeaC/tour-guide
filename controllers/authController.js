 ...
 * @param {*} options 
 */
const createAndSendToken = (user, statusCode, res, options = {}) => { // TODO: use this
...
exports.protect = catchAsync(async (req, res, next) => {
	...
	if (tokenHeader && tokenHeader.startsWith('Bearer')) token = tokenHeader.split(' ')[1];
	...
	if (changed) return next(new AppError("Your password changed. Please sing in again.", 401));
...
exports.restrictTo = (...params) => {
	return (req, res, next) => {
		if (!params.includes(req.user.role)) return next(new AppError("You are not allowed to delete this tour.", 403));
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
	...
	const link = `${req.protocol}://${req.get('host')}/api/v1/users/resetpassword/${tempToken}`;
...
exports.updatePassword = catchAsync(async (req, res, next) => { 
	...
	if (!req.body.password || !req.body.passwordConfirm) return next(new AppError('Please provide new password and confirmation password.', 400));
